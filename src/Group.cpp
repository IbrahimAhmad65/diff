...
Group::Group(const std::shared_ptr<Group> &g) { elements = g->elements; }
Group::Group(const Group *g) { elements = g->elements; }
...
Group::apply(std::vector<std::shared_ptr<Group>> &elements) const {
...
operator+(std::shared_ptr<Group> g1, T g2) {
  auto k = std::vector<std::shared_ptr<Group>>{
      g1, std::make_shared<Number>(Number(g2))};
  return std::make_shared<Addition>(Addition(k));
}
template <typename T>
requires std::is_arithmetic_v<T> std::shared_ptr<Group>
operator+(Group &g1, T g2) {
  auto k = std::vector<std::shared_ptr<Group>>{
      g1.clone(), std::make_shared<Number>(Number(g2))};
  return std::make_shared<Addition>(Addition(k));
}
template std::shared_ptr<Group> operator+
    <int>(std::shared_ptr<Group> g1, int g2);
template std::shared_ptr<Group> operator+
    <double>(std::shared_ptr<Group> g1, double g2);
